...
- **Corregido:** Aislado el proceso nativo de Whisper en un hilo de ThreadPool puro (`Task.Run` con `.ConfigureAwait(false)`) y modificado el `_processor` para instanciarse de manera efímera (`await using var`) por cada transcripción en lugar de reutilizarse, solucionando un crash grave originado en C++. Se añadió hook a `AppDomain.CurrentDomain.UnhandledException` para facilitar depuraciones futuras.
